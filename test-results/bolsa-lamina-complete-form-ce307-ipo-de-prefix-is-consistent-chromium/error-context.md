...
- Name: bolsa-lamina-complete-formats.spec.ts >> Field Naming Consistency - BOLSA vs LÁMINA >> Verify "Tipo de" prefix is consistent
- Location: tests\e2e\bolsa-lamina-complete-formats.spec.ts:212:3
...
> 222 |     await nameInput.fill('Test Tipo de');
  ...
  262 | 
  263 |     expect(oldFuelleLabels).toBe(0);
  264 |   });
  265 | });
  266 | 
```